...
{
    [Theory]
    [InlineData(new[] { true, true, true, true }, false)]
    [InlineData(new[] { true, true, true }, false)]
    [InlineData(new[] { true, true }, false)]
    [InlineData(new[] { true }, false)]
    [InlineData(new bool[] { }, false)]
    [InlineData(new[] { true, true, true, false }, true)]
    [InlineData(new[] { true, true, false, true }, true)]
    [InlineData(new[] { true, false, true, true }, true)]
    [InlineData(new[] { false, true, true, true }, true)]
    [InlineData(new[] { true, false, true, false }, true)]
    [InlineData(new[] { false, true, false, true }, true)]
    ...
    {
        // Arrange
        var challenger = new ChallengesSet03();

        // Act
        var actual = challenger.ArrayContainsAFalse(values);
    ...
    [InlineData(new[] { -1, -2, -3, -4, -5 }, true)]
    [InlineData(new[] { 1, 2, 2, 3, 4 }, false)]
    [InlineData(new[] { 1, -2, -2, 3, -4 }, false)]
    [InlineData(new[] { 1, -2, 2, 3, 4 }, false)]
    [InlineData(new[] { 1, 3, 5, 7, 9 }, true)]
    [InlineData(new[] { 1, 3, 5, 8 }, true)]
    [InlineData(new[] { 10, 10, 10, 10 }, false)]
    [InlineData(new[] { 0, 1, 0 }, true)]
    ...
    public void IsSumOfOddsOdd(IEnumerable<int> numbers, bool expected)
    {
        ...
        // Act
        var actual = challenger.IsSumOfOddsOdd(numbers);
    ...
    public void PasswordContainsUpperLowerAndNumber(string password, bool expected)
    {
        ...
        // Act
        var actual = challenger.PasswordContainsUpperLowerAndNumber(password);
    ...
    public void GetFirstLetterOfString(string str, char expected)
    {
        ...
        // Act
        var actual = challenger.GetFirstLetterOfString(str);
    ...
    public void GetLastLetterOfString(string str, char expected)
    {
        ...
        // Act
        var actual = challenger.GetLastLetterOfString(str);
    ...
    [InlineData(new[] { 0, 5, 10 }, 10)]
    [InlineData(new[] { 1, 2, 30 }, 29)]
    [InlineData(new[] { 1, 2, 3, 2, 1 }, 0)]
    [InlineData(new[] { -5, 1, 1, 1, 1, 5 }, 10)]
    [InlineData(new[] { -5, 1, 1, 1, 1, -5 }, 0)]
    [InlineData(new[] { 5, 1, 1, 1, 1, -5 }, -10)]
    [InlineData(new[] { 50, 1, 1, 1, 1, 5 }, -45)]
    [InlineData(new[] { 5, 1, 1, 1, 1, 50 }, 45)]
    public void LastMinusFirst(int[] numbers, int expected)
    {
        ...
        // Act
        var actual = challenger.LastMinusFirst(numbers);
    ...
    public void ShouldGetOddsBelow100()
    {
        ...
        // Act
        var actual = challenger.GetOddsBelow100();
    ...
    public void ChangeAllElementsToUppercase(string[] values, string[] expected)
    {
        ...
        for (var i = 0; i < values.Length; i++) Assert.Equal(expected[i], values[i]);
    }
}