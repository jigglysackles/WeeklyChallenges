...
public class ChallengesSet02Tests
{
    ...
    public void CharacterIsALetter()
    {
        ...
        {
            // Act
            var lowercaseActual = challenger.CharacterIsALetter(letter);
            var uppercaseActual = challenger.CharacterIsALetter(char.ToUpper(letter));
    ...
    public void CharacterIsNotALetter()
    {
        // Arrange
        var challenger = new ChallengesSet02();
        var nonLetters = "0123456789`~!@#$%^&*()-_=+[{]}\\|;:'\",<.>/?\u2222";

        // Check all letters
        foreach (var letter in nonLetters)
        {
            // Act
            var actual = challenger.CharacterIsALetter(letter);
    ...
    public void CountOfElementsIsEven(string[] strings, bool expected)
    {
        ...
        // Act
        var actual = challenger.CountOfElementsIsEven(strings);
    ...
    public void IsNumberOdd(int number, bool expected)
    {
        ...
        // Act
        var actual = challenger.IsNumberOdd(number);
    ...
    [InlineData(new[] { -15.5, 1, -5, 8 }, -7.5)]
    [InlineData(new[] { -2.7, 0, -2.3, 12, 9 }, 9.3)]
    ...
    [InlineData(new[] { -1, -2, -3, -4, -5 }, -6)]
    [InlineData(new[] { 1, -2, 3, -4, 5 }, -6)]
    [InlineData(new[] { 2, 2, 2, 2, 3, 3, 2 }, 10)]
    [InlineData(new[] { 2, 4, 2, 20, 9, 10, 2, 7 }, 40)]
    [InlineData(new[] { 0, 0, 0 }, 0)]
    [InlineData(new[] { 0, 2, 0 }, 2)]
    [InlineData(new[] { 1, 1, 1, 3, 3, 5, 5, 5, 5, 7 }, 0)]
    [InlineData(new int[] { }, 0)]
    [InlineData(null, 0)]
    public void SumEvens(int[] numbers, int expected)
    {
        ...
        // Act
        var actual = challenger.SumEvens(numbers);
